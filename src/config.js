...
var types =
  'armed_conflict,disease_outbreak,displacement,malnutrition,natural_disaster,search_and_rescue,other';
types = types.split(',');
...
export default {
  ...
  API_REPORTS_TIME_WINDOW_MAX:
    process.env.API_REPORTS_TIME_WINDOW_MAX || 604800, // 1w
  // API_EVENTS_LIMIT: process.env.API_EVENTS_LIMIT,
  API_EVENT_STATUS_TYPES: (
    process.env.API_EVENT_STATUS_TYPES || 'active,inactive'
  ).split(','),
  API_EVENT_STATUSES: (
    process.env.API_EVENT_STATUSES ||
    'monitoring,exploration,ongoing,intervention,complete'
  ).split(','),
  API_MSF_RESPONSE_TYPES: (
    process.env.API_MSF_RESPONSE_TYPES ||
    'direct_activities,remote_activities,coordination,donations'
  ).split(','),
  API_EVENT_TYPES: powerSet(types),
  API_OPERATIONAL_CENTERS: (
    process.env.API_OPERATIONAL_CENTERS || 'OCA,OCB,OCBA,OCG,OCP'
  ).split(','),
  API_COUNTRY_DETAIL_CATEGORY:
    process.env.API_COUNTRY_DETAIL_CATEGORY ||
    'map,strategy,security,mission,toolbox,document_repo'.split(','),
  API_COUNTRY_DETAIL_TYPES:
    process.env.API_COUNTRY_DETAIL_TYPES || 'link,file'.split(','),
  API_REPORT_STATUS_TYPES: (
    process.env.API_REPORT_STATUS_TYPES || 'unconfirmed,confirmed,ignored'
  ).split(','),
  AWS_COGNITO_PEM: (process.env.AWS_COGNITO_PEM || 'public_key').replace(
    /,/g,
    '\n'
  ),
  ...
  DEFAULT_EVENT_SEARCH_DISTANCE:
    process.env.DEFAULT_EVENT_SEARCH_DISTANCE || 1000000,
  ...
  STATIC_AUTH_RETURN_PATH:
    process.env.STATIC_AUTH_RETURN_PATH || 'public/authreturn',
  ...
  STATIC_RESOURCES_PATH:
    process.env.STATIC_RESOURCES_PATH || 'public/resources',
  STATIC_UNSUBSCRIBE_PATH:
    process.env.STATIC_UNSUBSCRIBE_PATH || 'public/unsubscribe',
  ...
  TABLE_EVENT_NOTIFICATIONS:
    process.env.TABLE_EVENT_NOTIFICATIONS || 'cognicity.event_notifications',
  TABLE_MSF_RESPONSES:
    process.env.TABLE_MSF_RESPONSES || 'cognicity.msf_responses',
  TABLE_COUNTRY_DETAILS:
    process.env.TABLE_COUNTRY_DETAILS || 'cognicity.country_details',
  ...
  TABLE_COUNTRY_LINKS:
    process.env.TABLE_COUNTRY_LINKS || 'cognicity.country_links',
