...
const ensureAuthenticated = (req, res, next) => {
  ...
  if (config.AZURE_AD_TENANT_NAME) {
    //Check if we are using azure ad auth
...
const ensureAuthenticatedWrite = (req, res, next) => {
  ...
  if (config.AZURE_AD_TENANT_NAME) {
    ...
    if (
      req.isAuthenticated() &&
      req.user._json.hasOwnProperty('groups') &&
      req.user._json.groups.indexOf(config.AZURE_AD_OPERATORS_GROUP_ID) > -1
    ) {
      return next();
    }
...
const inAsiaBBox = coords => {
  ...
  return lat > -12.5611 && lat < 82.5005 && lng > 19.6381 && lng < 180.0;
};
