...
export default (config, logger) => ({
  sendContactUpdateEmail: (recipient, theGUID) =>
    new Promise((resolve, reject) => {
      ...
      let uLink =
        config.BASE_URL + 'contact/?token=' + theGUID + '&email=' + recipient;

      let emContext = {
        updateLink: uLink,
        expiresIn: config.PEER_GUID_TIMEOUT / 3600
      };
  ...
  sendShareNotificationEmail: (
    ...
    new Promise((resolve, reject) => {
      ...
      request.get(
        'https://graph.microsoft.com/v1.0/users/' + sender_oid,
        {
          headers: {
        ...
        function(err, res, body1) {
          if (err) {
            ...
            request.get(
              'https://graph.microsoft.com/v1.0/users/' + recipient_oid,
              {
                headers: {
                  Authorization: 'Bearer ' + access_token,
                  'Content-Type': 'application/json'
              ...
              function(err, res, body2) {
                if (err) {
                  ...
                  let emContext = {
                    ...
                    contact_hyperlink:
                      config.BASE_URL + '#contact' + contact_data.id,
