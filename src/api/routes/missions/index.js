...
export default ({ config, db, logger }) => {
  ...
  api.put(
    ...
    (req, res, next) => {
      missions(config, db, logger)
        .updateMission(req.params.id, req.body)
        .then(data => {
  ...
  api.post(
    ...
    (req, res, next) => {
      events(config, db, logger)
        .ReActivateEvent(req.body.eventId)
        .then(data => {
          handleGeoResponse(data, req, res, next);
          ...
          missions(config, db, logger)
            ...
            .then(() => {
              logger.info(
                'mission ' + req.params.id + ' successfully deleted.'
              );
            })
            .catch(err => {
              /* istanbul ignore next */
