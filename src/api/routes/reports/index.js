...
export default ({ config, db, logger }) => {
  ...
  api.get(
    ...
    (req, res, next) =>
      reports(config, db, logger)
        .all(req.query.hasOwnProperty('eventId') ? req.query.eventId : null)
    ...
    validate({
      body: Joi.object().keys({
        eventId: Joi.number()
          ...
          .min(1),
        status: Joi.string()
          .valid(config.API_REPORT_STATUS_TYPES)
          .required(),
        created: Joi.date()
          .iso()
          .required(),
  ...
  api.post(
    '/sms',
    twilio.webhook(config.TWILIO_AUTH_TOKEN, {
      host: 'dev.msf-reach.org',
      protocol: 'https'
    }),
    (req, res, next) => {
  ...
  api.post(
    ...
    (req, res, next) => {
      reports(config, db, logger)
        .linkToEvent(req.params.id, req.body.eventId)
        .then(data => handleGeoResponse(data, req, res, next))
        .catch(err => {
          /* istanbul ignore next */
          logger.error(err);
