...
export default (config, db, logger) => ({
        ...
        function(err, response) {
          if (err) {
            ...
            let values = [
              ...
              { description: message, report_tag: type },
   ...
   */
  linkToEvent: (report_id, event_id) =>
    new Promise((resolve, reject) => {
      // Setup query
      let query = `UPDATE ${config.TABLE_REPORTS}
      SET event_id=$1,report_key=(SELECT report_key from ${
        config.TABLE_EVENTS
      } WHERE id=$1),status='unconfirmed' where id=$2
