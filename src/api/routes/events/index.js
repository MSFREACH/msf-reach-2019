...
export default ({ config, db, logger }) => {
  ...
  api.post(
    ...
    (req, res, next) => {
      ...
      events(config, db, logger)
        .unsubscribe(req.params.id, email)
  ...
  api.post(
    ...
    (req, res, next) => {
      events(config, db, logger)
        .unsubscribe(req.params.id, req.body.email)
        .then(data => handleResponse(data, req, res, next))
        .catch(err => {
          /* istanbul ignore next */
          logger.error(err);
  ...
  api.post(
    ...
    (req, res, next) => {
      events(config, db, logger)
        .subscribe(req.params.id, req.body.subscribers)
        .then(data => handleResponse(data, req, res, next))
        .catch(err => {
          /* istanbul ignore next */
          logger.error(err);
  ...
  api.post(
    ...
    validate({
      body: Joi.object().keys({
        status: Joi.string()
          .valid(config.API_EVENT_STATUS_TYPES)
          .required(),
        ...
        metadata: Joi.object()
          ...
          .keys({
            ...
            status_updates: Joi.array().items(
              Joi.object().keys({
                type: Joi.string(),
            ...
            severity_measures: Joi.array().items(
              Joi.object().keys({
                scale: Joi.number()
                  .min(1)
                  .max(3),
    ...
    (req, res, next) => {
      let userEmail =
        req.body.subscribe && req.user
          ? req.user._json.preferred_username
          : null;
      events(config, db, logger)
        .createEvent(req.body, userEmail)
        .then(data => handleGeoResponse(data, req, res, next))
        .catch(err => {
          /* istanbul ignore next */
          logger.error(err);
      ...
      events(config, db, logger)
        ...
        .then(data => {
          if (req.body.status === 'inactive') {
            ...
            missions(config, db, logger)
              .createMission(req.body, req.params.id)
              .then(data => handleGeoResponse(data, req, res, next))
              .catch(err => {
                /* istanbul ignore next */
                logger.error(err);
  ...
  api.put(
    ...
    (req, res, next) => {
      events(config, db, logger)
        .updateEventExtCapacity(req.params.id, req.body)
        .then(data => handleResponse(data, req, res, next))
        .catch(err => {
          logger.error(err);
          next(err);
  ...
  api.put(
    ...
    (req, res, next) => {
      events(config, db, logger)
        .updateEventFigures(req.params.id, req.body)
        .then(data => handleResponse(data, req, res, next))
        .catch(err => {
          logger.error(err);
          next(err);
  ...
  api.put(
    ...
    validate({
      ...
      body: Joi.object().keys({
        resources: Joi.object().keys({
          ...
          institutional_donors: Joi.array().items(
            Joi.object().keys({
    ...
    (req, res, next) => {
      events(config, db, logger)
        .updateEventResources(req.params.id, req.body)
        .then(data => handleResponse(data, req, res, next))
        .catch(err => {
          logger.error(err);
          next(err);
  ...
  api.post(
    ...
    (req, res, next) => {
      ...
      events(config, db, logger)
        .inviteToSubscribe(req.params.id, inviteData)
        .then(data => handleResponse(data, req, res, next))
        .catch(err => {
          /* istanbul ignore next */
          /* istanbul ignore next */
