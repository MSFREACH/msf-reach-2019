...
export default (config, db, logger) => ({
   ...
   */
  all: (status, country, location, types, search) =>
    new Promise((resolve, reject) => {
      ...
      let query = `SELECT id, status, type, created_at, updated_at, report_key as reportkey, metadata, ext_capacity as extCapacity, figures, resources, the_geom
            ...
            WHERE ($1 is null or status = $1) AND
                ...
                ($3 is null or ST_DWITHIN(ST_TRANSFORM(the_geom,3857),ST_TRANSFORM(ST_GEOMFROMTEXT($3,4326),3857),${
                  config.DEFAULT_EVENT_SEARCH_DISTANCE
                }))
   ...
   */
  createEvent: (body, subscribtionEmail) =>
    new Promise((resolve, reject) => {
      // Setup query
      ...
      if (config.GOOGLE_API_KEY) {
        request(
          'https://maps.googleapis.com/maps/api/geocode/json?latlng=' +
            String(body.location.lat) +
            ',' +
            String(body.location.lng) +
            '&key=' +
          ...
          function(error, response, response_body) {
            ...
            if (error) {
              ...
              if (
                ...
                geocoded.results &&
                geocoded.results[0] &&
                geocoded.results[0].address_components
              ) {
                ...
                for (let i = 0; i < address.length; i++) {
                  if (
                    address[i].types.indexOf('administrative_area_level_1') > -1
                  ) {
  ...
  updateEventExtCapacity: (id, body) =>
    new Promise((resolve, reject) => {
      // Setup query
      let query = `UPDATE ${config.TABLE_EVENTS}
            SET updated_at = now(),
      ...
      db.oneOrNone(query, values)
        ...
        .then(data =>
          resolve({
            id: String(id),
            status: data.status,
            extCapacity: data.extCapacity
          })
        )
        .catch(err => reject(err));
    }),

  updateEventFigures: (id, body) =>
    new Promise((resolve, reject) => {
      // Setup query
      let query = `UPDATE ${config.TABLE_EVENTS}
            SET updated_at = now(),
  ...
  activateEvent: body =>
    new Promise((resolve, reject) => {
      ...
      db.one(query, values)
        .timeout(config.PGTIMEOUT)
        .then(data =>
          resolve({ id: String(data.id), subscribers: data.subscribers })
        ) // eslint-disable-line no-unused-vars
        .catch(err => reject(err));
    }),

  ReActivateEvent: event_id =>
    new Promise((resolve, reject) => {
      // Setup query
      let query = `UPDATE ${config.TABLE_EVENTS}
			SET status = $1,
          ...
          function(error, response, response_body) {
            ...
            if (error) {
              ...
              if (
                ...
                for (
                  let i = 0;
                  i < geocoded.results[0].address_components.length;
                  i++
                ) {
                  if (
                    geocoded.results[0].address_components[i].types.indexOf(
                      'country'
                    ) > -1
                  ) {
                    country =
                      geocoded.results[0].address_components[i].long_name;
                  }
                }
              }
            }
  ...
  unsubscribe: (id, email) =>
    new Promise((resolve, reject) => {
      ...
      db.one(query, values)
        .timeout(config.PGTIMEOUT)
        .then(data =>
          resolve({ id: String(id), subscribers: data.subscribers })
        ) // eslint-disable-line no-unused-vars
        .catch(err => reject(err));
    }),

  inviteToSubscribe: (id, data) =>
    new Promise((resolve, reject) => {
      mail(config, logger)
        .emailInviteToSubscribe(data, id)
        .then(data => resolve(data))
        .catch(err => reject(err));
    }),
  ...
  deleteEvent: id =>
    new Promise((resolve, reject) => {
      ...
      db.any(query1, values)
        .then(() => db.oneOrNone(query2, values))
        .timeout(config.PGTIMEOUT)
        .then(data => resolve(data))
        .catch(err => reject(err));
    })
