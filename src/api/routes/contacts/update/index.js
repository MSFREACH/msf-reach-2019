...
export default ({ config, db, logger }) => {
  ...
  api.get(
    ...
    (req, res, next) => {
      ...
      update(config, db, logger)
        .requestGUID(req.query.email)
        .then(data => {
          if (data !== null) {
            logger.info('Email user ' + data.guid);
            mail(config, logger).sendContactUpdateEmail(
              req.query.email,
              data.guid
            );
            res.status(200).json({
              ...
              result: 'contact update link emailed to user',
              emailExists: true
            });
          } else {
  ...
  api.get(
    ...
    (req, res, next) => {
      ...
      update(config, db, logger)
        .validateGUID(req.query.email, req.query.guid)
        .then(data => {
          if (data !== null) {
            ...
            res.status(401).json({
              statusCode: 401,
              time: new Date().toISOString(),
              result: 'Contact GUID invalid or expired'
            });
          }
        })
        .catch(err => {
        ...
        .then(data => {
          if (data !== null) {
            ...
            contacts(config, db, logger)
              .updateContact(data.id, req.body)
              .then(() => {
                res.status(200).json({
                  statusCode: 200,
                  time: new Date().toISOString(),
                  result: 'updated contact successfully'
                });
              })
              .catch(err => {
                /* istanbul ignore next */
