...
export default ({ config, db, logger }) => {
  ...
  api.get(
    ...
    (req, res, next) =>
      eventNotifications(config, db, logger)
  ...
  api.post(
    ...
    (req, res, next) => {
      eventNotifications(config, db, logger)
        .createEventNotification(req.body)
        .then(data => handleResponse(data, req, res, next))
        .catch(err => {
          /* istanbul ignore next */
          logger.error(err);
  ...
  api.put(
    ...
    (req, res, next) => {
      eventNotifications(config, db, logger)
        .updateEventNotification(req.params.id, req.body)
        .then(data => handleResponse(data, req, res, next))
        .catch(err => {
          /* istanbul ignore next */
          logger.error(err);
  ...
  api.delete(
    ...
    (req, res, next) => {
      eventNotifications(config, db, logger)
        .deleteEventNotification(req.params.id)
        .then(() =>
          res.status(200).json({
            statusCode: 200,
            time: new Date().toISOString(),
            result: 'event notification deleted',
            id: req.params.id
          })
        )
        .catch(err => {
