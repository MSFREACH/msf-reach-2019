...
const init = (config, initializeDb, routes, logger) =>
  new Promise((resolve, reject) => {
    ...
    if (config.SESSION_SECRET) {
      ...
      ); //Hopefully this fixes #236 //TODO Need to save sessions to db instead to avoid memory leaks in prod
    } else {
    ...
    const removeUser = function(user) {
      for (var i = 0, len = users.length; i < len; i++) {
        if (
          typeof users[i] !== 'undefined' &&
          typeof user !== 'undefined' &&
          users[i].oid === user.oid
        ) {
          users.splice(i, 1);
          return;
        }
    ...
    initializeDb(config, logger)
      .then(db => {
          ...
          app.post(
            ...
            function(req, res) {
              ...
              res.cookie('userdisplayName', req.user.displayName, {
                maxAge: 1000 * 60 * 1
              }); //1 min cookie age should be enough
              res.cookie('email', req.user._json.preferred_username);
              res.cookie('oid', req.user.oid, { maxAge: 1000 * 60 * 1 });
              res.redirect('/authreturn');
            }
        ...
        app.use('/logout', function(req, res) {
          //Link in the navbar for logout links here when in Azure AD Auth Mode
          removeUser(req.user);
        ...
        app.use('/landing', [
          ...
          express.static(config.STATIC_PATH)
        ]);
