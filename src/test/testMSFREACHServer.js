...
describe('Cognicity Server Testing Harness', function() {
  ...
  it('Server fails if database connection not possible', function(done) {
    let config = {};
    init(config, initializeDb, routes, logger).catch(err => {
          ...
          test
            ...
            .end(function(err, res) {
              if (err) {
                ...
                eventId =
                  res.body.result.objects.output.geometries[0].properties.id;
                reportKey =
                  res.body.result.objects.output.geometries[0].properties
                    .reportkey;
                done();
              }
            });
          ...
          test
            ...
            .end(function(err, res) {
              if (err) {
                ...
                for (
                  let i = 0;
                  i < res.body.result.objects.output.geometries.length;
                  i++
                ) {
                  if (
                    res.body.result.objects.output.geometries[i].properties
                      .id === String(eventId)
                  ) {
                    output = true;
                  }
            ...
            .end(function(err, res) {
              if (err) {
                ...
                for (
                  ...
                  if (
                    res.body.result.objects.output.geometries[i].properties.id
                  ) {
                    output = true;
                    lat =
                      res.body.result.objects.output.geometries[i]
                        .coordinates[1];
                    lng =
                      res.body.result.objects.output.geometries[i]
                        .coordinates[0];
