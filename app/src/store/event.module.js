...
const actions = {
  ...
  [EDIT_EVENT_FIGURES]({ state }) {
    const currentKeyFigures = state.event.figures.keyFigures.filter(item => {
...
const getters = {
  ...
  eventTypes(state) {
    ...
    if (state.eventProperties && state.eventProperties.type) {
      const types = state.eventProperties.type
        .replace(/other:/g, '')
        .split(',');
  ...
  eventExtCapacity(state) {
    if (!state.event.extCapacity && state.event.metadata) {
      ...
      const ground = payload.ext_capacity_type_in_ground
        ...
        : null;
      const combined = [cap, action, human, ground].filter(function(el) {
        return el != null;
      });
      let capacities = [];
