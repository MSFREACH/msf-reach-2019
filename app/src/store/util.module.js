...
import {
  FETCH_URL_START,
  FETCH_UPLOAD_URL_END,
  FETCH_DOWNLOAD_URL_END,
  UPLOAD_START,
  UPLOAD_END,
...
const actions = {
  [FETCH_UPLOAD_URL]({ commit }, params) {
    ...
    return UtilService.getUpload(params).then(({ data }) => {
  ...
  [FETCH_DOWNLOAD_URL]({ commit }, params) {
    commit(FETCH_URL_START);
    return UtilService.getDownload(params).then(({ data }) => {
  ...
  [FETCH_BUCKET_URLS]({ commit }, params) {
    ...
    return UtilService.getBucketFiles(params).then(({ data }) => {
  ...
  [FETCH_REVERSE_GEOCODER]({ commit }, coordinate) {
    return UtilService.getReverseGeocoder(coordinate).then(({ data }) => {
...
const mutations = {
  ...
  [FETCH_GEOJSON_POLYGON_END](state, result) {
    const polygon = _.find(result, function(o) {
