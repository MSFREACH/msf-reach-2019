        ...
        <v-list>
          ...
          >
            <v-list-tile slot="activator">
              ...
              <v-list-tile-action>
                <v-checkbox v-model="checkedSections[item.value]" />
              </v-list-tile-action>
            </v-list-tile>
            <v-list-tile v-for="(subItem, i) in item.subItems" :key="i + 'i'">
              ...
              <v-list-tile-action>
                <v-checkbox
                  v-model="checkedFilters[item.value][i]"
                  :value="subItem.value"
                />
              </v-list-tile-action>
            </v-list-tile>
...
export default {
  ...
  methods: {
    ...
    initMap() {
      ...
      if (this.eventId) {
        const eventObj = eventFeatureCollection.filter(item => {
    ...
    loadEventsLayer() {
      const vm = this;
      const featureCollection = getFeatures(this.eventsGeoJson, 'output');
    ...
    zoomToEventCenter() {
      const eventObj = this.eventFeatureCollection.filter(item => {
        return item.properties.id == this.eventId;
      });
